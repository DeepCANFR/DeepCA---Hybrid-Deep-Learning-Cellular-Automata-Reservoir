...
template[4,0] = 1
template[4,-1] = 1
EE_dendritic_arbor_parameters["projection_template"] = template
...
EE_delay_line_parameters["delay"] = 1.5
EE_delay_line_parameters["time_step"] = time_step

EI_dendritic_arbor_parameters = {}
EI_dendritic_arbor_parameters["projection_template"] = cp.ones((3,3))
...
IE_dendritic_arbor_parameters["projection_template"] = cp.ones((3,3))
...
# Initiate components
excitatory_somas = rm.Simple_Integrate_and_fire_soma(excitatory_soma_parameters)

inhibitory_somas = rm.Simple_Integrate_and_fire_soma(inhibitory_soma_parameters)
inhibitory_somas.set_dead_cells(cp.random.uniform(0,1,population_size)<0.2)

EE_delay_line = rm.Delay_Line(EE_delay_line_parameters)
EE_dendritic_arbor = rm.Dendritic_Arbor(EE_dendritic_arbor_parameters)
EE_axonal_terminal = rm.Dynamical_Axonal_Terminal_Markram_etal_1998(excitatory_to_excitatory_dynamical_synapse_parameters)
EE_dendritic_spine = rm.Dendritic_Spine_Maas(E_dendritic_Spine_parameters)

EI_delay_line = rm.Delay_Line(EI_delay_line_parameters)
EI_dendritic_arbor = rm.Dendritic_Arbor(EI_dendritic_arbor_parameters)
EI_axonal_terminal = rm.Dynamical_Axonal_Terminal_Markram_etal_1998(excitatory_to_inhibitory_dynamical_synapse_parameters)
EI_dendritic_spine = rm.Dendritic_Spine_Maas(E_dendritic_Spine_parameters)

IE_delay_line = rm.Delay_Line(IE_delay_line_parameters)
IE_dendritic_arbor = rm.Dendritic_Arbor(IE_dendritic_arbor_parameters)
IE_axonal_terminal = rm.Dynamical_Axonal_Terminal_Markram_etal_1998(inhibitory_to_excitatory_dynamical_synapse_parameters)
IE_dendritic_spine = rm.Dendritic_Spine_Maas(E_dendritic_Spine_parameters)
...
######################################################################################
'''
EE_delay_line.interface_read_variable(excitatory_somas.current_spiked_neurons)
EE_dendritic_arbor.interface(EE_delay_line.current_spike_output)
EE_axonal_terminal.interface_read_variable(EE_dendritic_arbor.current_spike_array)
EE_dendritic_spine.interface_read_variable(EE_axonal_terminal.current_synaptic_response)
excitatory_somas.interface_input(EE_dendritic_spine.current_synaptic_output)

EI_delay_line.interface_read_variable(excitatory_somas.current_spiked_neurons)
EI_dendritic_arbor.interface(EI_delay_line.current_spike_output)
EI_axonal_terminal.interface_read_variable(EI_dendritic_arbor.current_spike_array)
EI_dendritic_spine.interface_read_variable(EI_axonal_terminal.current_synaptic_response)
inhibitory_somas.interface_input(EI_dendritic_spine.current_synaptic_output)

IE_delay_line.interface_read_variable(inhibitory_somas.current_spiked_neurons)
IE_dendritic_arbor.interface(IE_delay_line.current_spike_output)
IE_axonal_terminal.interface_read_variable(IE_dendritic_arbor.current_spike_array)
IE_dendritic_spine.interface_read_variable(IE_axonal_terminal.current_synaptic_response)
excitatory_somas.interface_input(IE_dendritic_spine.current_synaptic_output)

excitatory_somas.interface_input(background_current)
excitatory_somas.interface_input(stimulation_input)

inhibitory_somas.interface_input(background_current)



network = [EE_dendritic_arbor, EE_delay_line, EE_axonal_terminal, excitatory_somas, inhibitory_somas, EI_dendritic_arbor, EI_axonal_terminal, EI_delay_line, IE_dendritic_arbor, IE_axonal_terminal, IE_delay_line, EE_dendritic_spine, EI_dendritic_spine, IE_dendritic_spine]
...
# set starting somatic voltage to noise
excitatory_somas.current_somatic_voltages[:,:] = cp.random.uniform(13.5, 15, population_size)
inhibitory_somas.current_somatic_voltages[:,:] = cp.random.uniform(13.5, 15, population_size)
...
for class_nr in range(10):
    ...
    readout_list[class_nr].interface_read_variable(excitatory_somas.current_spiked_neurons)
...
for epoch in range(nr_of_epochs):
    ...
    for image_nr in range(len(image_train)):
        ...
        stimulation_input[:,:,0] = (cp.array(image_train[image_nr,:,:])/255)*18
        ...
        for trial_time_step in range(timesteps_per_image):
            ...
            if cp.any(excitatory_somas.current_spiked_neurons > 0) and (trial_time_step+1)*time_step > 50:
            ...
            image = np.concatenate((image, image_train[image_nr,:,:]), axis = 1)
