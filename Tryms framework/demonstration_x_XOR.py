...
population_1 = fm.Soma_AS_with_projection_weights(membrane_decay = 0.4, treshold_decay = 0.3, membrane_treshold_resting_distance = 0.5, population_size_x = pop_x, population_size_y = pop_y, neighbourhood_template = neighboyrhood_template_1, weight_mean= 1/900, weight_SD = 0.3)
population_2 = fm.Soma_AS_with_projection_weights(membrane_decay = 0.4, treshold_decay = 0.3, membrane_treshold_resting_distance = 0.5, population_size_x = pop_x, population_size_y = pop_y, neighbourhood_template = neighboyrhood_template_1, weight_mean= 1/5, weight_SD = 0.3)
population_3 = fm.Soma_AS_with_projection_weights(membrane_decay = 0.4, treshold_decay = 0.3, membrane_treshold_resting_distance = 0.5, population_size_x = pop_x, population_size_y = pop_y, neighbourhood_template = neighboyrhood_template_1, weight_mean= 1/5, weight_SD = 0.3)
...
while True:
    ...
    for index, pop in enumerate(pop_list):
        ...
        pop.compute_spikes()
        pop.send_spikes_to_synapses(pop.spiked_neurons)
        pop.treshold_additive_input(np.sum(pop_list[index-1].spike_array, 2)**2)
