if __name__ == '__main__':
    ...
    simulation_length = spike_trains.shape[1]
    ...
    for distance_nr in range(3):
        for pair_nr in range(200):
            for train_nr in range(2):
                with Client(n_workers = 6) as client:
                    print("workers initialized")
                    train_name = r"\distance_ " + str(distances[distance_nr]) + "pair_nr_" + str(pair_nr) + "_train_" + str(train_nr) + ".npy"
                    time.sleep(1)
                    filename = train_path_name + train_name
                    stimulation_train = ncp.load(filename)
                    simulation_length = len(stimulation_train)

                    print("loaded spike train: ", train_name)
                    ...
                    with open("homogenous_Circuit_equation_somas_1.pkl", 'rb') as handle:
                    ...
                    if stop_simulation == False:
                        filename = train_name[2:-3] + "_network_history_1_homogenous_Circuit_equation_somas_1"
