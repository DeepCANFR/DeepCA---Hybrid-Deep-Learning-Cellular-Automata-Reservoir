...
time_step = 1
population_size = (1000,1000) #image_train[0,:,:].shape
upper_limit = 10000
...
I_soma_parameters["resting_potential_variable"] = {"distribution":"Izhikevich", "base_value":0.025, "multiplier_value": -0.05, "dependent":"membrane_recovery"} # b in model, sensitivity of recovery variable. Affects the resting potential, should be between -70 and -60 mV depending on the value of b
...
#E_soma_parameters["reset_recovery_variable"] = {"distribution":"homogenous", "value":4} # d in model, after spike reset of the recovery variable u
I_soma_parameters["reset_recovery_variable"] = {"distribution":"homogenous", "value":-65, "dependent":False}
...
if __name__ == '__main__':
    ...
    with Client(n_workers = 6) as client:
        ...
        neuron_populations = [E1_neurons, I1_neurons]
        input_populations =[E1_input, I1_input]
        ...
        import cloudpickle
        with open('in_vitro_network.pkl', 'wb') as output:
            cloudpickle.dump(network, output)

        with open('in_vitro_inputs.pkl', 'wb') as output:
