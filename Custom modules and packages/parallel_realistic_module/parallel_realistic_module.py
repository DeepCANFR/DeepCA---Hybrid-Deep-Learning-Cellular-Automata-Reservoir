...
class Izhivechik_Equation(object):
    ...
    def __call__(self, V, t = 0):
        delta_v_u = ncp.zeros((self.population_size[0], self.population_size[1], 2))
        delta_v_u[:,:,0] = 0.04*V[:,:,0]**2 + 5*V[:,:,0] + 140 - V[:,:,1] + self.summed_inputs
        delta_v_u[:,:,1] = self.a * (self.b * self.v - V[:,:,1])
        return delta_v_u
...
class Base_Integrate_and_Fire_Soma(Component):
    ...
    def __init__(self, parameter_dict):
        ...
        self.current_somatic_voltages = ncp.zeros(self.parameters["population_size"])
        self.new_somatic_voltages = ncp.zeros(self.parameters["population_size"])
...
class Izhikevich_Soma(Base_Integrate_and_Fire_Soma):
    ...
    def set_membrane_function(self):
        membrane_function = Izhivechik_Equation(self.parameters["input_resistance"], self.parameters["membrane_time_constant"], self.summed_inputs)
        self.membrane_solver = RungeKutta2_cupy(membrane_function, self.parameters["time_step"])

    def compute_new_values(self):
        self.time_since_last_spike += self.time_step

        delta_v_u = self.membrane_solver.advance(self.current_somatic_voltages, t = 0)
        ...
        self.new_spiked_neurons[:,:] = self.new_somatic_voltages > self.threshold
        self.new_u += self.new_spiked_neurons*self.parameters["Izhikevich"]["d"]
...
# LEgacy
class Simple_Integrate_and_Fire_Soma(Component):
    interfacable = 0
    current_somatic_voltages = 0
    current_spiked_neurons = 0
    new_spiked_neurons = 0
    ...
    def compute_new_values(self):
        ...
        # set this to avoid overlflow
        below_upper_limit = self.time_since_last_spike < self.upper_limit
        self.time_since_last_spike *= below_upper_limit
        self.time_since_last_spike += (below_upper_limit == 0)*self.upper_limit
...
class Dynamical_Axonal_Terminal_Markram_etal_1998(Component):
    ...
    def compute_new_values(self):
        ...
        self.new_utilization_of_synaptic_efficacy[:,:,:] = self.current_utilization_of_synaptic_efficacy * ncp.exp((-self.time_since_last_spike) / self.tau_facil) + self.resting_utilization_of_synaptic_efficacy*(1 - self.current_utilization_of_synaptic_efficacy * ncp.exp((-self.time_since_last_spike) / self.tau_facil))



        self.new_neurotransmitter_reserve[:,:,:] = self.current_neurotransmitter_reserve * (1 - self.new_utilization_of_synaptic_efficacy)*ncp.exp(-self.time_since_last_spike / self.tau_recovery) + 1 - ncp.exp(-self.time_since_last_spike / self.tau_recovery)
...
class Dendritic_Spine_Maas(Component):
    ...
    def compute_new_values(self):
        ...
        self.new_synaptic_output[:,:,:] = self.last_input_since_spike * ncp.exp(-self.time_since_last_spike / self.time_constant)
    ...
    def update_current_values(self):
        ...
        self.current_synaptic_input[:,:,:] = self.external_component.interfacable
...
class Neurons(object):
    def __init__(self, soma_parameter_dict, position, name, client):
        self.client = client

        self.components = {}
        self.name = name
        self.components[self.name] = self.client.submit(Simple_Integrate_and_Fire_Soma, soma_parameter_dict, actors = True)
    ...
    def interface_futures(self, parameter_dict, neuron):
        ...
        if "delay_line" in parameter_dict:
            ...
            name["ID"] = parameter_dict["dendritic_spines"]["ID"]
            connection.append(name)
            self.component[name["ID"]] = self.client.submit(Dendritic_Spine_Maas, parameter_dict["dendritic_spines"], actors = True)
...
class Input_Class(object):
    ...
    def __init__(self, population_size):
        self.inputs = ncp.zeros(population_size)
        self.interfacable = self.inputs
